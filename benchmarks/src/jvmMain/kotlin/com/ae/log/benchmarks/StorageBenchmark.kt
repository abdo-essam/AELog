...
/**
 * JMH benchmarks for storage throughput, measured through the public [InMemoryPluginStorage] API.
 *
...
open class StorageBenchmark {
    ...
    @Setup
    fun setup() {
        storage = InMemoryPluginStorage(capacity)
        // Pre-fill to steady-state so we benchmark the overwrite (wrap-around) path
        repeat(capacity) { storage.add("init-$it") }
    }

    /**
     * Cost of one [InMemoryPluginStorage.add]:
     *   lock + list drop/copy + StateFlow emit.
     *
     * This is the hot path for EVERY log call in the library.
     * Target: < 20µs at capacity=500.
     */
    @Benchmark
    fun addSingle(): InMemoryPluginStorage<String> {
        storage.add("benchmark-entry")
        return storage
    }

    /**
     * Cost of reading the current snapshot from [InMemoryPluginStorage.dataFlow].
    ...
    /**
     * [InMemoryPluginStorage.updateFirst] — used by [NetworkStorage] to match a pending
     ...
     */
    @Benchmark
    fun updateFirstWorstCase(): InMemoryPluginStorage<String> {
    ...
    /**
     * [InMemoryPluginStorage.addOrReplace] when the predicate does NOT match.
     ...
     */
    @Benchmark
    fun addOrReplaceNewEntry(): InMemoryPluginStorage<String> {
    ...
    /**
     * [InMemoryPluginStorage.addOrReplace] when the predicate MATCHES the first element.
     * Best-case scan (O(1) match). Used when a network response completes a
     * pending request that was just added.
     */
    @Benchmark
    fun addOrReplaceExistingFirstElement(): InMemoryPluginStorage<String> {
        storage.addOrReplace(
