...
class PluginStorageTest {
    ...
    @Test
    fun `add - emits new list via dataFlow`() = runTest {
        storage.add("a")
        storage.add("b")
    ...
    @Test
    fun `add - evicts oldest when over capacity`() = runTest {
        storage.add("a")
        storage.add("b")
        storage.add("c")
    ...
    @Test
    fun `clear - resets dataFlow to empty`() = runTest {
        storage.add("a")
        storage.add("b")
        storage.clear()
    ...
    @Test
    fun `addOrReplace - replaces matching element by predicate`() = runTest {
        storage.add("apple")
        storage.add("banana")
    ...
    @Test
    fun `addOrReplace - adds new element when predicate does not match`() = runTest {
        storage.add("apple")
    ...
    @Test
    fun `updateFirst - transforms matching element`() = runTest {
        storage.add("hello")
        storage.add("world")
    ...
    @Test
    fun `updateFirst - is no-op when no element matches`() = runTest {
        storage.add("hello")
